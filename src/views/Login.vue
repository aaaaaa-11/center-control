      ...
      <div class="login-page-panel__body">
        ...
        >
          ...
          <a-form-item name="code">
            <a-input
              ...
              :placeholder="
                beforeGetCode ? '请输入验证码' : '控制台打印的验证码'
              "
              v-model:value="formState.code"
            >
              <template #suffix>
                <span
                  class="text-link get-code"
                  v-show="beforeGetCode"
                  @click="getCode"
                  :class="!validatePhone ? 'disabled-code' : ''"
                >
                  获取验证码
                </span>
                <span class="reget-code" v-show="!beforeGetCode">
                  重新获取({{ second }})s
                </span>
              </template>
            </a-input>
        ...
        <div class="policy">
          <a-tooltip :visible="!policyChecked">
            ...
            <a-checkbox
              class="fs13 privite-policy"
              v-model:checked="policyChecked"
            >
              我已阅读并同意
              <span
                class="text-link fs13"
                @click="$router.push('/privitepolicy')"
              >
                《用户隐私政策》
              </span>
            </a-checkbox>
        ...
        <a-button
          type="primary"
          class="btn btn-blue fs15 block login-btn"
          @click="submit"
        >
          登录
        </a-button>
        <div class="text-center text-link">
          <span
            v-show="formType === FormTypes.ACCOUNT"
            @click="changeType(FormTypes.PHONE)"
          >
            手机号验证码登录
          </span>
          <span
            v-show="formType === FormTypes.PHONE"
            @click="changeType(FormTypes.ACCOUNT)"
          >
            账号密码登录
          </span>
        </div>
...
const handleLoginRes = (res: any, returnToken: boolean = false) => {
  ...
  if (code === 0) {
    ...
    try {
      ...
      // 如果 没有从其他页面 跳过来的，或从 没有权限的页面 跳过来
      if (!page || (page && !permission.includes(`${page}:view`))) {
        page = permission
          .find((i: string) => i.includes(':view'))
          ?.replace(':view', '') // 找到第一个有权限的页面
      }
      if (!page) {
...
const submit = () => {
  ...
  formRef.value
    ...
    .then(() => {
      const params =
        formType.value === FormTypes.PHONE
          ? {
              phone: formState.phone,
              uuid: uuid.value,
          ...
          : {
              ...
              password: formatPw(formState.password),
            }
      userLogin(params)
        .then((res) => {
